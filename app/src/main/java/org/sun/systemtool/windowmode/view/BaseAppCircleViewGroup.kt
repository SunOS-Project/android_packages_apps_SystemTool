...
import org.sun.systemtool.windowmode.util.Config.CIRCLE_CENTER_X_LAND
import org.sun.systemtool.windowmode.util.Config.CIRCLE_CENTER_X_PORT
import org.sun.systemtool.windowmode.util.Config.CIRCLE_CENTER_Y_LAND
import org.sun.systemtool.windowmode.util.Config.CIRCLE_CENTER_Y_PORT
import org.sun.systemtool.windowmode.util.Config.ICON_SIZE_RATIO
import org.sun.systemtool.windowmode.util.Config.getCircleMaxIconArray
import org.sun.systemtool.windowmode.util.Config.getCircleRadiusRatioArray
import org.sun.systemtool.windowmode.util.Shared.navbarHeight
...
abstract class BaseAppCircleViewGroup(
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        val circleMaxIconArray = getCircleMaxIconArray(count)
        val circleRadiusRatioArray = getCircleRadiusRatioArray(count)

        for (i in 0 until count) {
            val circleIdx = circleMaxIconArray.indexOfFirst { i + 1 <= it }
            val circleCount = circleMaxIconArray.indexOfFirst { count <= it }
            val currentCircleTotal = if (circleIdx < circleCount) {
                if (circleIdx == 0) {
                    circleMaxIconArray[0]
                } else {
                    circleMaxIconArray[circleIdx] - circleMaxIconArray[circleIdx - 1]
                }
            } else {
                if (circleIdx == 0) {
                    count
                } else {
                    count - circleMaxIconArray[circleIdx - 1]
                }
            }
            val currentCircleIdx = if (circleIdx == 0) {
                i + 1
            } else {
                i + 1 - circleMaxIconArray[circleIdx - 1]
            }
